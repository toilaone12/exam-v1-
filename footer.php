<!-- footer -->
<footer class="page-footer">
    <div class="footer content">
        <div class="widgetplus widgetplus-block slider-0 lazyloaded" id="widgetplus-block-1673628463">
        <div class="block-static clearfix">
            <div id="footer-widgets" class="footer-widgets">
            <div class="container-fluid">
                <div class="footer-main">
                <div class="row" style="margin-left: 0; margin-right: 0;">
                    <div class="col-12 col-md-6 col-lg-3">
                    <div class="footer-links-wrap">
                        <div class="h4 title">LIEN HE</div>
                        <div class="footer__info">
                        <ul>
                            <li>
                            <a href="mailto:hoctiengduccungdecamy@gmail.com"><i class="fas fa-envelope"></i>echhocbai@gmail.com</a>
                            </li> 
                            <li>
                            <a href="tel:0963.041.210 -  0934.143.128"><i class="fas fa-phone-volume"></i>0963.041.210 -  0934.143.128</a>
                            </li> 
                            <li>
                            <a href="javascript:;"><i class="fas fa-map-marker-alt"></i>Số 2, Ngõ 18, Nguyễn Cơ Thạch, Cầu Diễn, Hà Nội</a>
                            </li>
                        </ul>
                        </div>
                    </div>
                    </div>
                    <div class="col-12 col-md-6 col-lg-3">
                    <div class="footer-links-wrap toggle-mobile">
                        <div class="h4 title">DANH MUC<i class="mbi mbi-chevron-down"></i></div>
                        <div class="footer-container content-toggle">
                        <ul class="footer-links">
                            <li><a href="#pro">Khóa học</a></li>
                            <li><a href="thuvien.php">Tu vung</a></li>
                            <li><a href="thuvien.php">Ngu phap</a></li>
                            <li><a href="#blog">Tin tuc</a></li>
                        </ul>
                        </div>
                    </div>
                    </div>
                    <div class="col-12 col-md-6 col-lg-3">
                    <div class="footer-links-wrap toggle-mobile">
                        <div class="h4 title">THANH TOAN<i class="mbi mbi-chevron-down"></i></div>
                        <div class="footer-container content-toggle footer-widgets__payment"><img loading="lazy" src="https://cpn.vn/pub/media/images/payment-visa.jpg"> <img loading="lazy" src="https://cpn.vn/pub/media/images/payment-master.jpg"> <img loading="lazy" src="https://cpn.vn/pub/media/images/payment-jcb.jpg"> <img loading="lazy" src="https://cpn.vn/pub/media/images/payment-atm.jpg"> <img loading="lazy" src="https://cpn.vn/pub/media/images/payment-tra-gop.jpg"> <img loading="lazy" src="https://cpn.vn/pub/media/images/payment-cod.jpg"></div>
                    </div>
                    
                    </div>
                    <div class="col-12 col-md-6 col-lg-3">
                    <div class="footer-links-wrap toggle-mobile">
                        <div class="h4 title">KET NOI<i class="mbi mbi-chevron-down"></i></div>
                        <div class="footer-container content-toggle footer-widgets__social"><a href="//fb.com/online.cpn" target="_blank" rel="nofollow noopener noreferrer"><img loading="lazy" src="https://cpn.vn/pub/media/images/social-facebook.png"></a> <a href="//www.youtube.com/@cpnvietnam" target="_blank" rel="nofollow noopener noreferrer"><img loading="lazy" src="https://cpn.vn/pub/media/images/social-youtube.png"></a> <a href="//www.tiktok.com/@dienmaycpn" target="_blank" rel="nofollow noopener noreferrer"><img loading="lazy" src="https://cpn.vn/pub/media/images/social-tiktok.png"></a> <a href="//zalo.me/3532659710685650526" target="_blank" rel="nofollow noopener noreferrer"><img loading="lazy" src="https://cpn.vn/pub/media/images/social-zalo.png"></a></div>
                    </div>
                    
                    </div>
                </div>
                </div>
                <div class="footer-copyright-vertical">
                <div class="row align-items-center" >
                    <div class="footer-copyright ">
                    <div class="text-copyright">© nguyenhongphuong20211663</div>
                    </div>
                </div>
                </div>
            </div>
            </div>            
        </div>   
        </div>
    </div>
    </footer> 
<div class="group-contact">
    <a href="" class="contact-message">
        <i class="text-white fa-brands fa-facebook-messenger"></i>
    </a>
    <a href="" class="contact-message">
        <i class="text-white fa-solid fa-phone-flip"></i>
    </a>
</div>
<button type="button" id="back-to-top">
    <i class="fa-solid fa-angle-up fs-18 fw-bold" style="line-height: 37px;"></i>
</button>