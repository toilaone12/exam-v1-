...
$result = mysqli_query($conn,'SELECT * FROM debai WHERE id = '.$id);
...
<?php
    if(isset($_POST) && $_POST){
        ...
        if($_POST['update']){
            ...
            $sql = "UPDATE `debai` SET name = '$name', list_question = '$jsonList' WHERE id = ".$id;
            // var_dump($sql); die;
            $insert = mysqli_query($conn,$sql);
            if($insert){
                // echo json_encode(['res' => 'success','title' => 'Thông báo thêm đề bài', 'icon' => 'success', 'text' => 'Thêm đề bài thành công']);            
                echo json_encode(['res' => 'success','title' => 'Thông báo sửa đề bài', 'icon' => 'success', 'text' => 'Sửa đề bài thành công']);
                exit;
            }else{
                echo json_encode(['res' => 'error','title' => 'Thông báo sửa đề bài', 'icon' => 'error', 'text' => 'Sửa đề bài thất bại']);
                exit;
            }
        }
    }
...
<div class="container-fluid">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form class="change-assignment" data-id="<?=$id?>">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="name">Tên đề tài</label>
                        <textarea name="ckeditor" id="" cols="30" rows="10" required><?=$row['name']?></textarea>
                    </div>
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <?php
                                ...
                                $sqlQuestion = "SELECT * FROM cauhoi";
                                $resultQuestion = mysqli_query($conn,$sqlQuestion);
                                while($rowQuestion = mysqli_fetch_assoc($resultQuestion)){   
                                    $isSelected = in_array($rowQuestion['id'], $array);
                            ?>
                            <option value="<?=$rowQuestion['id']?>" <?= $isSelected ? 'selected' : '' ?>>
                                <?=$rowQuestion['name']?>
                            </option>
                            <?php } ?>
                            </select>
